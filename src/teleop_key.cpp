  ...
  void readOne(char *c)
  {
    ...
    for (;;)
    {
      ...
      if (events > 0)
      {
        ...
        else if (buffer.Event.KeyEvent.wVirtualKeyCode == 0x20)
        {
          *c = KEYCODE_SPACE;
          return;
        }
...
TeleopBugina::TeleopBugina() : linear_(0),
                               angular_(0),
                               l_scale_(1.0),
                               a_scale_(1.0)
{
...
int main(int argc, char **argv)
{
  ...
  TeleopBugina teleop_bugina;

  signal(SIGINT, quit);

  teleop_bugina.keyLoop();
  quit(0);

  return (0);
}

void TeleopBugina::keyLoop()
{
  char c;
  bool dirty = false;
  linear_ = angular_ = 0;
  ...
  puts("---------------------------");
  puts("Use arrow keys to move the mbot. 'q' to quit.");
  ...
  for (;;)
  {
    ...
    if(linear_ > 1) linear_ = 1;
    else if(linear_ < -1) linear_ = -1;

    if(angular_ > 1) angular_ = 1;
    else if(angular_ < -1) angular_ = -1;
